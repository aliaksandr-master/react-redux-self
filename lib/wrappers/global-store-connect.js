...
const applyReducer = (reducer, state, action) => {
  if (Array.isArray(reducer)) {
    return reducer.reduce((currentState, reducer) => {
      if (state === undefined) {
        return { ...currentState, ...reducer(state, action) };
      }
...
const selfReducer = (state = {}, action) => {
  ...
  return {
    ...state,
    [selfID]: applyReducer(reducer, state[selfID], action)
  };
};
